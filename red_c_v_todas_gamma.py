...
# interacion=1
try:
    matriz_evaluate=np.load('./mejora_imagen/c_v_pesos_'+name_r+'/matriz_c_v.npy')
    matriz_evaluate_train=np.load('./mejora_imagen/c_v_pesos_'+name_r+'/matriz_c_v_train.npy')
    matriz_evaluate_test=np.load('./mejora_imagen/c_v_pesos_'+name_r+'/matriz_c_v_test.npy')
...
os.makedirs("./mejora_imagen/c_v_pesos_"+name_r+"/", exist_ok=True)  
for grupo in range(4,numeros_grupos): #Falta la 5, se lleno la ram

    print('entrenando modelo '+name_r)
    interacion=grupo+1
    datos_entrenamiento = './mejora_imagen/data_c_v/interaccion'+str(grupo+1)+'/train'
    datos_validacion='./mejora_imagen/data_c_v/interaccion'+str(grupo+1)+'/test'
    datos_test='./mejora_imagen/test'
    ...
    #                               mode='min')
    checkpointer_val = ModelCheckpoint(filepath=os.path.join("./mejora_imagen/c_v_pesos_"+name_r+"/val_accuracy_"+name_r+"_best_cnn"+str(interacion)+".weights.h5"),
                                  monitor='val_accuracy',
                                  verbose=1,
                                  save_best_only=True,
                                  save_weights_only=True,
    ...
    cnn.save('./mejora_imagen/c_v_pesos_'+name_r+'/modelo_'+name_r+'.weights.h5')
    ...
    for i in range(alpha_inicio,alpha_final):
            ...
            os.makedirs('./mejora_imagen/c_v_history_'+name_r+'/', exist_ok=True)

            np.save('./mejora_imagen/c_v_history_'+name_r+'/c_v'+str(interacion)+'his_accuracy_'+str(i)+'.npy', his_accuracy)
            np.save('./mejora_imagen/c_v_history_'+name_r+'/c_v'+str(interacion)+'his_val_accuracy_'+str(i)+'.npy',his_val_accuracy)
            np.save('./mejora_imagen/c_v_history_'+name_r+'/c_v'+str(interacion)+'his_loss_'+str(i)+'.npy',his_loss)
            np.save('./mejora_imagen/c_v_history_'+name_r+'/c_v'+str(interacion)+'his_val_loss_'+str(i)+'.npy',his_val_loss)
            

            
            cnn.load_weights("./mejora_imagen/c_v_pesos_"+name_r+"/val_accuracy_"+name_r+"_best_cnn"+str(interacion)+".weights.h5")
    ...
    np.save('./mejora_imagen/c_v_pesos_'+name_r+'/matriz_c_v.npy', matriz_evaluate)
    np.save('./mejora_imagen/c_v_pesos_'+name_r+'/matriz_c_v_train.npy', matriz_evaluate_train)
    np.save('./mejora_imagen/c_v_pesos_'+name_r+'/matriz_c_v_test.npy', matriz_evaluate_test)
